 ...
 */
@Service
public class NumberParamHandler implements ParamHandler<TagMarkTask>
{
    @Override
    public List<TagInstance> handle(TagMarkTask task)
    {
        ...
        Map<Long, String> custValueMap = new HashMap<>(JobHandlerConstants.MAX_SERIAL_TASK_NUM);
        ...
        if (CollectionUtils.isNotEmpty(paramDtoList))
        {
            for (TagParamDto paramDto : paramDtoList)
            {
                ...
                {
                    Map<Long, Boolean> executeResult = getRuleHandler().execute(ruleDtoList, task, custValueMap);
                    ...
                    executeResult.forEach((custId, ifMark) -> {
                        if (ifMark)
                        {
                            ...
                            instance.setTagValue(custValueMap.get(custId));
